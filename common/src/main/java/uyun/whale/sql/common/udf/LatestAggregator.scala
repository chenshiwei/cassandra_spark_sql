...
case class LatestAggregator() extends UserDefinedAggregateFunction {

  import org.apache.spark.sql.types._

  override def inputSchema: StructType = StructType(Array(StructField("input", StringType), StructField("timestamp", LongType)))

  override def bufferSchema: StructType = StructType(Array(StructField("later", StringType), StructField("timestamp", LongType)))
